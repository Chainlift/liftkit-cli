...
export async function initCommand() {
	try {
        ...
        switch (true) {
            case fileExists(tsconfigPath) && tsconfigPathsMatch(readJsonFile(tsconfigPath), config.tsconfigjson):
        ...
        switch (true) {
            case fileExists(pkgPath): {
                ...
                if (pkg && (!pkg.scripts || !pkg.scripts['add'] || pkg.scripts['add'] !== 'liftkit add')) {
                ...
                const hasShadcn = pkg && pkg.devDependencies && pkg.devDependencies['shadcn'] === '2.7.0';
                switch (!hasShadcn) {
                    case true:
                        console.log(
                            '\x1b[32m%s\x1b[0m',
...
export async function addCommand(component: string) {
	...
	try {
		...
		if (registryItem.registryDependencies && registryItem.registryDependencies.length > 0) {
			console.log(chalk.yellow(`Processing ${registryItem.registryDependencies.length} dependencies...`));
			await processDependencies(
				registryItem.registryDependencies,
				fetch,
				processor as { validate: (item: unknown) => { isValid: boolean; errors: string[]; warnings: string[] } },
				registryProcessor as unknown as { processRegistryItem: (item: unknown) => Promise<void> },
		...
		console.log(chalk.green(`✓ Successfully added component: ${registryItem.name}`));
		console.log(chalk.gray(`Files processed: ${result.processedFiles.length}`));
		if (result.npmDependencies.length > 0 || result.devDependencies.length > 0) {
...
export async function processDependencies(
  ...
  processor: { validate: (item: unknown) => { isValid: boolean; errors: string[]; warnings: string[] } },
    ...
    try {
      ...
      if (!depValidation.isValid) {
        ...
        if (depValidation.errors) {
          depValidation.errors.forEach((error: string) => logError(`  - ${error}`));
        }
