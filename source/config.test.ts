...
describe('config', () => {
  it('should export a config object with all required properties', () => {
    ...
    expect(config).toHaveProperty('templaterepo', 'https://raw.githubusercontent.com/Chainlift/liftkit-template/refs/heads/main');
  ...
  it('should have tsconfigjson.compilerOptions.paths as an object with string[] values', () => {
    ...
    Object.values(paths).forEach(val => {
      ...
      (val as string[]).forEach((entry: unknown) => expect(typeof entry).toBe('string'));
    });
  });
}); 