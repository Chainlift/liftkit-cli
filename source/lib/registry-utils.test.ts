...
describe('registry-utils', () => {
    ...
    it('extractPropertyDetails', () => {
      ...
      expect(details).toEqual(
        expect.arrayContaining([
          expect.objectContaining({ name: 'foo', type: 'string', required: true }),
          expect.objectContaining({ name: 'bar', type: 'string', enum: ['1', '2'] }),
          expect.objectContaining({ name: 'baz', type: 'array', items: { type: 'string' } }),
        ])
      );
    });
    it('getSchemaInfo', () => {
      const info = utils.getSchemaInfo(schema);
      expect(info).toHaveProperty('schemaVersion', 'http://json-schema.org/draft-07/schema#');
  ...
  describe('validation', () => {
    ...
    it('createFieldValidator validates types and enums', () => {
      ...
      expect(utils.createFieldValidator(schema, 'foo', 123)).toEqual(["Field 'foo' must be a string"]);
      expect(utils.createFieldValidator(schema, 'bar', '1')).toEqual([]);
      expect(utils.createFieldValidator(schema, 'bar', '3')).toEqual(["Field 'bar' must be one of: 1, 2"]);
    ...
    it('validateRequiredFields finds missing fields', () => {
      expect(utils.validateRequiredFields(schema, { name: 'test', type: 'registry:component', foo: 'x' } as RegistryItem)).toEqual([]);
      expect(utils.validateRequiredFields(schema, { name: 'test', type: 'registry:component' } as RegistryItem)).toEqual([
        'Missing required field: foo',
      ]);
    });
    it('validateFieldTypes finds type errors', () => {
      expect(utils.validateFieldTypes(schema, { name: 'test', type: 'registry:component', foo: 1 } as RegistryItem)).toContain(
        "Field 'foo' must be a string"
      );
    });
    it('findUnknownFields finds unknown fields', () => {
      expect(utils.findUnknownFields(schema, { name: 'test', type: 'registry:component', foo: 'x', qux: 1 } as RegistryItem)).toEqual([
        'Unknown field: qux',
      ]);
    });
    it('validateRegistryItem returns isValid, errors, warnings', () => {
      const result = utils.validateRegistryItem(schema, { name: 'test', type: 'registry:component', foo: 1, qux: 2 } as RegistryItem);
  ...
  describe('change planning', () => {
    ...
    it('createChangePlan returns a plan object', () => {
      const schema = { required: [], properties: { a: { type: 'string' }, name: { type: 'string' }, type: { type: 'string' } } };
      const current = { a: 'x', name: 'test', type: 'registry:component' } as RegistryItem;
      const desired = { a: 'y', b: 'z', name: 'test', type: 'registry:component' } as RegistryItem;
...
describe('registry-utils (additional coverage)', () => {
  it('formatChange, formatAddition, formatRemoval, formatValidationIssue', () => {
    ...
    expect(utils.formatValidationIssue({ type: 'current', issues: ['err'] })).toMatch(/current/);
  });
  ...
  it('getTypeDefaults returns defaults for known and unknown types', () => {
    expect(utils.getTypeDefaults('registry:component')).toHaveProperty('dependencies');
  ...
  it('getRegistryTypes and getFileTypes extract enums from schema', () => {
    const schema = {
      properties: {
        ...
        files: { type: 'array', items: { type: 'object', properties: { type: { type: 'string', enum: ['x', 'y'] } } } },
      },
    };
  ...
  it('planChangesForItems plans changes for an array of items', () => {
    const schema = { required: [], properties: { name: { type: 'string' }, foo: { type: 'string' } } };
    const items: RegistryItem[] = [
