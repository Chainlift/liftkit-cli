...
describe('RegistryProcessor', () => {
  ...
  beforeEach(() => {
    processor = new RegistryProcessor({ baseDir: '/tmp', installDependencies: false });
  });
  ...
  it('processes all files in item.files and skips nulls', async () => {
    const item = {
      ...
      files: [
        ...
        { path: 'skip', content: null, type: 'registry:component' as RegistryType },
  ...
  it('appends CSS variables to globals.css', async () => {
    ...
    const processor = new RegistryProcessor({ baseDir: '/tmp' }, mockFs as unknown as typeof import('fs'));
