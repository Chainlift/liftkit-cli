...
import type { RegistryItem, RegistrySchema, SchemaProperty, RegistryType } from './registry-types.js';

// Remove typedEntries and typedKeys helpers

export function pipe<T, R>(...fns: Array<(value: T) => T | R>): (value: T) => R {
  return (value: T): R => fns.reduce((acc: T | R, fn: (value: T) => T | R) => fn(acc as T), value) as R;
}

export function curry<T extends unknown[], R>(fn: (...args: T) => R): (...args: T) => R | ((...args: unknown[]) => unknown) {
  return function curried(...args: T): R | ((...args: unknown[]) => unknown) {
    if (args.length >= fn.length) {
      return fn(...args);
    } else {
      return (...next: unknown[]): unknown => curried(...([...args, ...next] as T));
    }
  };
}
...
export const extractPropertyNames = (schema: RegistrySchema): string[] => {
  return Object.keys(schema.properties) as Array<keyof typeof schema.properties> as string[];
};

export const extractRequired = (schema: RegistrySchema): string[] => (schema.required ? schema.required.map((key: string) => key) : []);

export const extractPropertyDetails = (schema: RegistrySchema): Array<{
  name: string;
  type: string;
  description?: string;
  required: boolean;
  ...
  const keys = Object.keys(schema.properties) as Array<keyof typeof schema.properties>;
  return keys.map((key) => {
...
export const getSchemaInfo = (schema: RegistrySchema): {
...
export const createFieldValidator = (
  ...
  const fieldSchema = schema.properties[fieldName as keyof typeof schema.properties];
  if (!fieldSchema) {
    ...
    case 'array':
      if (!isArray(value)) {
        ...
        (value as unknown[]).forEach((item: unknown, index: number) => {
          if (!isString(item)) {
            errors.push(`Array item at index ${index} in field '${fieldName}' must be a string`);
          }
        });
      }
      break;
...
export const validateFieldTypes = (schema: RegistrySchema, item: RegistryItem): string[] => {
  const errors: string[] = [];
  ...
  for (const field of schemaFields) {
    if (field in item) {
      const fieldErrors = createFieldValidator(schema, field, item[field as keyof RegistryItem]);
...
export const findUnknownFields = (schema: RegistrySchema, item: RegistryItem): string[] => {
...
export const findModifications = (
  ...
  for (const key in desired) {
    if (Object.prototype.hasOwnProperty.call(desired, key) &&
      ...
      changes.push({ field: key, from: current[key], to: desired[key], type: 'modification' as const });
    }
  }
  return changes;
};
...
export const findAdditions = (
  ...
  for (const key in desired) {
    if (Object.prototype.hasOwnProperty.call(desired, key) &&
        !Object.prototype.hasOwnProperty.call(current, key)) {
      additions.push({ field: key, value: desired[key], type: 'addition' as const });
    }
  }
  return additions;
};
...
export const findRemovals = (
  ...
  for (const key in current) {
    if (Object.prototype.hasOwnProperty.call(current, key) &&
      ...
      removals.push({ field: key, value: current[key], type: 'removal' as const });
    }
  }
  return removals;
};
...
export const createChangePlan = (
  ...
  const changes = findModifications(current as unknown as Record<string, unknown>, desired as unknown as Record<string, unknown>);
  const additions = findAdditions(current as unknown as Record<string, unknown>, desired as unknown as Record<string, unknown>);
  const removals = findRemovals(current as unknown as Record<string, unknown>, desired as unknown as Record<string, unknown>);
  const resultingItem = applyChanges(current as unknown as Record<string, unknown>, desired as unknown as Record<string, unknown>, removals);
  ...
  const validResultingItem: RegistryItem = {
    ...
    type: (resultingItem['type'] as RegistryType) || (current.type as RegistryType),
    ...resultingItem,
  };
  ...
  return {
    ...
    summary: generateChangeSummary({ changes, additions, removals, validationIssues }),
  };
};

// Formatting functions
export const formatChange = (change: { field: string; from: unknown; to: unknown }): string => {
  return `Modified ${change.field}: ${change.from} → ${change.to}`;
};

export const formatAddition = (addition: { field: string; value: unknown }): string => {
...
export const formatValidationIssue = (issue: { type: string; issues: string[] }): string => {
...
export const generateChangeSummary = (plan: {
  ...
  if (plan.validationIssues.length > 0) {
    sections.push(`Validation issues: ${plan.validationIssues.map(formatValidationIssue).join(', ')}`);
  }
...
export const validateItems = (
  schema: RegistrySchema,
  items: RegistryItem[]
): Array<{ item: RegistryItem; validation: { isValid: boolean; errors: string[]; warnings: string[] } }> => {
...
export const createSchemaProcessor = (schema: RegistrySchema) => {
  return {
    ...
    planChanges: (currentItem: RegistryItem, desiredChanges: Partial<RegistryItem>) => createChangePlan(schema, currentItem, { ...currentItem, ...desiredChanges }),
    generateTemplate: (type?: RegistryType) => generateTemplate(type ?? 'registry:component'),
